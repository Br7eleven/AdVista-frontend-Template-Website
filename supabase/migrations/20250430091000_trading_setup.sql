...
DROP POLICY IF EXISTS "Trading pairs are viewable by all users" ON public.trading_pairs;
...
DROP POLICY IF EXISTS "Trading candles are viewable by all users" ON public.trading_candles;
...
DROP POLICY IF EXISTS "Users can insert their own trades" ON public.user_trades;
