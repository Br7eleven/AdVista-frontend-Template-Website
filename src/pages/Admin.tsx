...
export default function Admin() {
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-500 dark:text-gray-400">Loading secure admin data...</div>;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-dark-600 p-6 rounded-lg shadow-sm border border-gray-100 dark:border-dark-500">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-light">Total Users</h2>
          </div>
          <div className="text-3xl font-bold text-gray-900 dark:text-light">{users.length}</div>
        </div>

        <div className="bg-white dark:bg-dark-600 p-6 rounded-lg shadow-sm border border-gray-100 dark:border-dark-500">
          <div className="flex items-center space-x-3 mb-4">
            <Clock className="w-6 h-6 text-primary-600 dark:text-primary-400" />
            <h2 className="text-lg font-semibold text-gray-900 dark:text-light">Pending Withdrawals</h2>
          </div>
          <div className="text-3xl font-bold text-gray-900 dark:text-light">
        ...
        <div className="bg-white dark:bg-dark-600 p-6 rounded-lg shadow-sm border border-gray-100 dark:border-dark-500">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-light">Total Paid Out</h2>
          </div>
          <div className="text-3xl font-bold text-gray-900 dark:text-light">
              ...
              {withdrawals.map((w: any) => (
                <tr key={w.id} className="hover:bg-gray-50 dark:hover:bg-dark-700/50">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900 dark:text-light">{w.user?.name || 'Unknown'}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">{w.user?.email}</div>
                  </td>
                  <td className="px-6 py-4 text-sm font-bold text-gray-900 dark:text-light">${w.amount.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 uppercase">{w.payment_method}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      w.status === 'completed' ? 'bg-accent-100 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300' :
                      w.status === 'pending' ? 'bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300' :
                    ...
                    {w.status === 'pending' && (
                      <>
                        <button
                          ...
                          className="p-1 text-accent-600 dark:text-accent-400 hover:bg-accent-50 dark:hover:bg-accent-900/20 rounded transition-colors"
                          title="Approve"
                        >
                          <CheckCircle size={20} />
                        </button>
                        <button
                          ...
                          className="p-1 text-secondary-600 dark:text-secondary-400 hover:bg-secondary-50 dark:hover:bg-secondary-900/20 rounded transition-colors"
                          title="Reject"
                        >
                          <XCircle size={20} />
                        </button>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-dark-500">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-dark-700/50">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900 dark:text-light">{user.name || 'User'}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">{user.email}</div>
                  </td>
                  <td className="px-6 py-4 text-sm font-bold text-accent-600 dark:text-accent-400">${user.balance?.toFixed(2) || '0.00'}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                    {new Date(user.created_at).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      user.is_admin ? 'bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300' : 'bg-gray-100 dark:bg-dark-500 text-gray-700 dark:text-gray-300'
                    }`}>
